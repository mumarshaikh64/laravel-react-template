    ...
    <div className='rounded px-4 py-4 mt-4'>
      ...
      <div className='bg-[#fff] px-4 py-4 mx-4 mt-5 rounded shadow-md'>
        <table className='w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400'>
          ...
          {
            mainContext?.subscriber == null ? <div>Please Wait...</div>
              : mainContext?.subscriber?.length > 0 ?
                (
                  <tbody>
                    {
                      mainContext?.subscriber?.map((d: any, index: number) => {
                        return <tr key={index}>
                          ...
                          <td scope="col" className="px-6 py-3">
                            <div className="max-w-[25rem] overflow-x-auto">
                              <span className="inline-block whitespace-nowrap">{d?.content}</span>
                            </div>
                          </td>
